...
from src.growth.proof_of_help import make_card
...
class Grid:
    def __init__(self, agents: list[Agent] | None = None) -> None:
        self.agents = agents or [SchemeAgent(), HospitalAgent(), MoneyAgent()]
    ...
    def handle(self, utterance: str, user_id: str = "demo-user", consent: ConsentFn = _auto_consent) -> dict:
        ...
        for task in tasks:
            ...
            entry = self.ledger.record(
                ...
                undo=(lambda d=action.description: None) if action.reversible else None,
            )
            spoken.append(
