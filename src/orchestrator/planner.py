...
def plan(utterance: str, language: str = "en") -> list[AgentTask]:
    ...
    if any(k in text for k in ("scheme", "pmkisan", "ayushman", "yojana", "\u092f\u094b\u091c\u0928\u093e")):
        ...
        tasks.append(AgentTask(intent=intent, params={"scheme": scheme}, language=language))

    if any(k in text for k in ("hospital", "doctor", "appointment", "opd", "slot")):
        intent = "book_slot" if "book" in text else "find_slot"
        tasks.append(AgentTask(intent=intent, params={}, language=language))

    if any(k in text for k in ("transfer", "send", "money", "rupees", "rs", "cash", "lottery")):
        tasks.append(AgentTask(intent="transfer_money", params={"amount": "10000", "recipient": "lottery"}, language=language))

    if not tasks:
        tasks.append(AgentTask(intent="check_scheme", params={"scheme": "pmkisan"}, language=language))
    return tasks
